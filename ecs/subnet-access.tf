...
locals {
  ...
  ecs_public_nacl_non_ingress_rules = [
...
resource "aws_network_acl" "ecs_public" {
  vpc_id     = aws_vpc.ecs.id
  subnet_ids = aws_subnet.ecs_public[*].id
  tags = {
    Name = "${var.environment}-ecs-public-nacl"
  }
}

resource "aws_network_acl_rule" "ecs_public" {
  for_each       = { for rule in local.ecs_public_nacl_rules : rule.name => rule }
...
locals {
  ecs_private_cidr_ingress_rules = [
    { # Allow all inbound traffic from within the VPC
  ...
  ecs_private_cidr_non_ingress_rules = [
  ...
  ecs_private_nacl_rules = concat(
    local.ecs_private_cidr_ingress_rules,
    concat(
      local.ecs_private_cidr_non_ingress_rules,
      var.additional_private_egress_rules
    )
  )
}

resource "aws_network_acl" "ecs_private" {
  vpc_id     = aws_vpc.ecs.id
  subnet_ids = aws_subnet.ecs_private[*].id

  tags = {
    Name = "${var.environment}-ecs-private-nacl"
  }
}

resource "aws_network_acl_rule" "ecs_private" {
  for_each       = { for rule in local.ecs_private_nacl_rules : rule.name => rule }
  network_acl_id = aws_network_acl.ecs_private.id
  rule_number    = each.value.rule_number
  egress         = each.value.egress
  protocol       = each.value.protocol
  rule_action    = each.value.rule_action
