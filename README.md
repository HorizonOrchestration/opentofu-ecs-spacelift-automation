...
| <a name="input_ecs_services"></a> [ecs\_services](#input\_ecs\_services) | Map of ECS services to create | <pre>map(object({<br/>    container_image       = string<br/>    container_name        = string<br/>    container_port        = optional(number)<br/>    task_cpu              = optional(number, 256)<br/>    task_memory           = optional(number, 512)<br/>    environment_variables = optional(map(string), {})<br/>    secrets               = optional(map(string), {})<br/>    health_check = optional(object({<br/>      command      = list(string)<br/>      interval     = number<br/>      timeout      = number<br/>      retries      = number<br/>      start_period = number<br/>    }))<br/>    mount_points = optional(list(object({<br/>      source_volume  = string<br/>      container_path = string<br/>      read_only      = bool<br/>    })), [])<br/>  }))</pre> | `{}` | no |
