...
| <a name="input_additional_public_egress_rules"></a> [additional\_public\_egress\_rules](#input\_additional\_public\_egress\_rules) | Additional NACL egress rules for public subnets | <pre>list(object({<br/>    name        = string<br/>    rule_number = number<br/>    egress      = bool<br/>    protocol    = string<br/>    rule_action = string<br/>    cidr_block  = string<br/>    from_port   = number<br/>    to_port     = number<br/>  }))</pre> | `[]` | no |
| <a name="input_allowed_public_ingress_cidrs"></a> [allowed\_public\_ingress\_cidrs](#input\_allowed\_public\_ingress\_cidrs) | List of CIDR blocks allowed to access public subnets on HTTPS (443) | `list(string)` | <pre>[<br/>  "154.51.81.155/32"<br/>]</pre> | no |
...
| <a name="input_capacity_provider"></a> [capacity\_provider](#input\_capacity\_provider) | ECS capacity provider to use (FARGATE or FARGATE\_SPOT) | `string` | `"FARGATE_SPOT"` | no |
...
| <a name="input_default_task_cpu"></a> [default\_task\_cpu](#input\_default\_task\_cpu) | Default CPU units for service tasks (256, 512, 1024, 2048, 4096) | `number` | `256` | no |
| <a name="input_default_task_memory"></a> [default\_task\_memory](#input\_default\_task\_memory) | Default memory for service tasks in MB (512, 1024, 2048, etc.) | `number` | `512` | no |
| <a name="input_ecs_services"></a> [ecs\_services](#input\_ecs\_services) | Map of ECS services to create | <pre>map(object({<br/>    container_image       = string<br/>    container_name        = string<br/>    container_port        = optional(number)<br/>    task_cpu              = optional(number)<br/>    task_memory           = optional(number)<br/>    environment_variables = optional(map(string))<br/>    secrets               = optional(map(string))<br/>    health_check = optional(object({<br/>      command      = list(string)<br/>      interval     = number<br/>      timeout      = number<br/>      retries      = number<br/>      start_period = number<br/>    }))<br/>    mount_points = optional(list(object({<br/>      source_volume  = string<br/>      container_path = string<br/>      read_only      = bool<br/>    })))<br/>    efs_volumes = optional(list(object({<br/>      name            = string<br/>      file_system_id  = string<br/>      root_directory  = string<br/>      access_point_id = string<br/>    })))<br/>  }))</pre> | `{}` | no |
| <a name="input_enable_cloudwatch_logging"></a> [enable\_cloudwatch\_logging](#input\_enable\_cloudwatch\_logging) | Whether to enable CloudWatch logging for VPC Flow Logs | `bool` | `false` | no |
